...
public class CarDaoImpl implements CarDao {
    ...
    private final String SQL_GET_CAR_BY_ID = "select * from car where car_id=?";
    private final String SQL_GET_CAR_BY_REGISTRATION_NUMBER = "select * from car where registration_number=?";
    private final String SQL_DELETE_CAR = "update car set is_deleted=1 where car_id = ?";
    private final String SQL_UPDATE_CAR = "update car set available=?, registration_number=? where car_id = ?";
    private final String SQL_UPDATE_CAR_AVAILABLE = "update car set available=? where car_id = ?";
    private final String SQL_SAVE_CAR = "insert into car(available,registration_number,car_profile_id) values (?,?,?)";
    ...
    public Optional<Car> findByRegistrationNumber(String registrationNumber) {
        try {
            return Optional.of(Objects.requireNonNull(jdbcTemplate.queryForObject(SQL_GET_CAR_BY_REGISTRATION_NUMBER, new CarMapper(), registrationNumber)));
        }
    ...
    public List<Car> getCarsByAvailable(boolean available) {
        try {
            ...
            LOGGER.error("Any cars which are "+(available?"available":"not available")+" are not found",ex.getCause());
    ...
    public Long save(Car car) {
        try {
            return (long) jdbcTemplate.update(SQL_SAVE_CAR, car.isAvailable(),car.getRegistrationNumber(),car.getCarProfileId());
        }
        catch (DataAccessException ex){
