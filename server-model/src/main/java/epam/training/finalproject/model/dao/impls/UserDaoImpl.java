...
public class UserDaoImpl implements UserDao {
    ...
    private final String SQL_FIND_BY_USERNAME = "select u.idUser, u.username, u.password, u.email, u.active from user u where u.username=?";
    private final String SQL_FIND_BY_EMAIL = "select u.idUser, u.username, u.password, u.email, u.active from user u where u.email=?";
    private final String SQL_GET_USER_BY_ID = "select u.idUser, u.username, u.password, u.email, u.active from user u where u.idUser=?";
    private final String SQL_DELETE_USER = "update user set active=? where idUser = ?";
...
//            return jdbcTemplate.queryForObject(SQL_FIND_BY_USERNAME, new UserMapper(), username);
    ...
    public Long delete(User user) {
        try {
            return (long) jdbcTemplate.update(SQL_DELETE_USER, user.isActive(), user.getId());
        }
        catch (DataAccessException ex){
            LOGGER.error("User with id "+user.getId()+" and username "+user.getUsername()+" has invalid credentials",ex.getCause());
            return -1L;
